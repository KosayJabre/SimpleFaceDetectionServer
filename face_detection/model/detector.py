...
from .utils import batched_decode, decode_landm, to_cuda
...
class RetinaNetDetector:
    def __init__(self, nms_iou_threshold: float, device: torch.device, max_resolution: int, fp16_inference: bool):
    ...
    def detect(self, images: np.ndarray, params: RetinaNetDetectorRequestParameters = RetinaNetDetectorRequestParameters()) -> tuple[np.ndarray, np.ndarray]:
    ...
    def _pre_process(self, image: np.ndarray) -> torch.Tensor:
        ...
        if self.max_resolution:
            ...
            image = torch.nn.functional.interpolate(image[None], size=size, mode='bilinear', align_corners=False)
